...
local BASE_URL = 'https://api.telegram.org/bot' .. config.api.token
local api = {} 
...
function send_inline(chat_id, text, keyboard)
	...
	local url = BASE_URL.."/sendMessage?chat_id="..chat_id.."&text="..URL.escape(text).."&parse_mode=Markdown&disable_web_page_preview=true&reply_markup="..URL.escape(responseString)
	local res, code, desc = sendRequest(url)
	if not res and code then --if the request failed and a code is returned (not 403 and 429)
		log_error('sendinline', code, {text}, desc)
	end
	return  res, code
end
...
function api.sendAdmin(text, markdown)
	return api.sendMessage(config.bot_owner, text, markdown)
end
